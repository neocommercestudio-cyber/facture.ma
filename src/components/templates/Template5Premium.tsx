...
export default function Template5Premium({ data, type }: TemplateProps) {
  const { user } = useAuth();
...
<div className="relative">
          ...
          <tbody>
            {data.items.map((item, index) => (
              <tr key={index} className="border-t border-[#0a1f44]">
                ...
                <td className="px-4 py-2 text-center">{item.quantity.toFixed(3)}</td>
  ...
  {(() => {
    ...
    return vatRates.map((rate) => (
      ...
      {/* FOOTER */}
      <div className="bg-[#0a1f44] text-white p-6 text-center text-sm">
        <p>
         <strong> {user?.company.name} </strong>  | {user?.company.address} | <strong>Tél :</strong> {user?.company.phone} | <strong>ICE :</strong> {user?.company.ice} |  <strong> IF:</strong> {user?.company.if} | <strong> RC:</strong> {user?.company.rc} | <strong> CNSS:</strong> {user?.company.cnss} | <strong> Patente :</strong> {user?.company.rc} | <strong> EMAIL :</strong> | <strong> SITE WEB  :</strong> 
        </p>
      </div>
    </div>
  );
